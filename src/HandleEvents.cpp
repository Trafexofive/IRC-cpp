...
void CoreServer::WelcomeClient() {
  ...
  clients[fd_c].set_response(":wesuseARCH.com NOTICE * :Please authenticate "
                             "with PASS, then set your NICK and USER.\r\n");
}

void CoreServer::HandleEvents(int fd, std::string buffer) {
...
void CoreServer::WriteEvent(int fd) {
  ...
  if (!clients[fd].get_response().empty()) {
    k = write(fd, clients[fd].get_response().c_str(),
              clients[fd].get_response().length());
    if (k < 0) {
