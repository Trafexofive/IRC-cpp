...
static std::string constructPrivmsg(const std::string &source,
                                    ...
                                    std::string message) {
  std::string privMsg =
      ":" + source + " PRIVMSG " + dest + " " + message + CRLF;
  return privMsg;
}
...
void CoreServer::send_message_to_channel(int fd, const std::string &channel,
  ...
  if (!chan->isMember(clients[fd])) {
    std::string msg =
        formatResponse(ERR_NOTONCHANNEL, "You're not on that channel");
    std::cout << formatServerMessage("ERROR", "Client not in channel")
              << std::endl;
    send(fd, msg.c_str(), msg.size(), 0);
    return;
  }
...
void CoreServer::send_message_to_user(int fd, const std::string &target,
                                      const std::string &message) {
  ...
  for (; it != clients.end(); ++it) {
    if (it->second.getNickName() == target) {
      if (it->second.isStatus(STATUS::DISCONNECTED))
        break;
      std::string msg =
          constructPrivmsg(clients[fd].getTarget(), target, message);
      send(it->first, msg.c_str(), msg.size(), 0);
      found = true;
      break;
    }
...
bool validmsg(std::vector<std::string> target, std::string message) {
  ...
  for (; it != target.end(); ++it) {
    if (it->empty() || it->at(0) == ' ' || it->at(0) == ',' ||
        it->at(it->size() - 1) == ',' || it->at(it->size() - 1) == ' ')
      return false;
  }
  return true;
}
...
void CoreServer::cmdPrivmsg(int fd, std::vector<std::string> &args) {
  if (args.size() < 3) {
    std::string msg =
        formatResponse(ERR_NEEDMOREPARAMS, "Not enough parameters");
    send(fd, msg.c_str(), msg.size(), 0);
    return;
  }
