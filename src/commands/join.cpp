...
void CoreServer::joinChannel(Client &client, const std::string &channelName)
{
	...
	channelPtr = getChannel(channelName);
	if (!channelPtr)
	{
	...
	if (channel.isPrivate() && !channel.isMember(client))
	{
		...
		client.setResponse(ERR_BADCHANNELKEY_MSG(client.getTarget(),
				channelName));
		return ;
	}
	if (channel.isMember(client))
	{
		...
		client.setResponse(ERR_USERONCHANNEL_MSG(client.getTarget(),
				channelName));
		return ;
	}
	channel.addMember(&client);
	this->broadcast(channel, formatBroadcastMessage(client.getTarget(), "JOIN",
			channel.getName()));
	if (!channel.getTopic().empty())
		client.setResponse(formatResponse(RPL_TOPIC, channel.getName() + " :"
				+ channel.getTopic()));
	// name reply
}
	...
	const std::string &key)
{
	...
	if (channel.isPrivate() && !channel.validatePassword(key))
	{
		std::cout << formatServerMessage("WARNING",
			"JOIN failed: Invalid channel key") << std::endl;
		client.setResponse(formatResponse(ERR_BADCHANNELKEY, channelName
	...
	this->broadcast(channel, formatBroadcastMessage(client.getTarget(), "JOIN",
			channel.getName()));
	if (!channel.getTopic().empty())
		client.setResponse(formatClientResponse(client.getTarget(), RPL_TOPIC,
				':' + channel.getTopic()));
	// channel name list reply
}
